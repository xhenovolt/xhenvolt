 ...
 * TiDB / MySQL seed.
 *
 * Idempotent for upserts (settings, systems via slug), destructive for
 * straight-insert tables (clearTable -> INSERT). Re-run safely after
 * editing — every list is fully rewritten from the source data here.
 *
 *   npm run db:seed
 *
 * The data here is the canonical first-load content for the public site
 * and the admin UI. Edits made later through /admin are authoritative —
 * re-running this seed will wipe non-upsert tables, so only re-run when
 * you intend to reset.
 */
...
/** Convert Postgres-style $N placeholders to mysql2 ? placeholders. */
function pgToMy(sql: string): string {
...
async function q<T = Record<string, unknown>>(
  ...
  const [rows] = (await pool.query(pgToMy(sql), params)) as [T[], unknown];
  return rows;
}
...
/**
 * Insert a row into `table` with a freshly-generated UUID id.
 * Returns the new id so callers can wire foreign keys without an extra
 * SELECT round-trip (MySQL has no RETURNING).
 */
async function qInsert(
  table: string,
  cols: string[],
  vals: unknown[],
): Promise<string> {
  ...
  const escaped = allCols.map((c) => `\`${c}\``).join(",");
  await pool.query(
    `INSERT INTO \`${table}\` (${escaped}) VALUES (${placeholders})`,
    [id, ...vals],
  );
  return id;
}
...
/**
 * Upsert against `conflictCol` (assumed UNIQUE). On insert, generates a
 * UUID; on conflict, updates the non-key columns. Then SELECTs the
 * canonical id via the conflict column — handles both branches.
 */
async function qUpsert(
  table: string,
  conflictCol: string,
  cols: string[],
  vals: unknown[],
  ...
  const placeholders = allCols.map(() => "?").join(",");
  const escaped = allCols.map((c) => `\`${c}\``).join(",");
  // Update every non-key column on conflict. id stays whatever it was.
  const updates = cols
...
async function seedSettings() {
  ...
  const items: Array<[string, unknown, string]> = [
    ...
    [
      ...
      {
        name: "Xhenvolt",
        tagline: "Real digital infrastructure for African institutions",
...
async function seedNavigation() {
  ...
  for (let i = 0; i < top.length; i++) {
    ...
    const id = await qInsert(
      "navigation_links",
      ["label", "href", "location", "sort_order", "is_external"],
      [t.label, t.href, "primary", i, false],
    );
    parents[t.label] = id;
  }
  ...
  for (const [parent, label, href] of drops) {
    await qInsert(
      "navigation_links",
      ["label", "href", "parent_id", "location", "sort_order", "is_external"],
      [label, href, parents[parent], "primary", order++, false],
    );
  }
}
...
async function seedFooter() {
  ...
  for (const [col, label, href, external] of cols) {
    await qInsert(
      "footer_links",
      ["label", "href", "column", "sort_order", "is_external"],
  ...
  for (let i = 0; i < socials.length; i++) {
    ...
    await qInsert(
      "social_links",
      ["platform", "label", "href", "icon", "sort_order"],
...
async function seedServices() {
  ...
  for (let i = 0; i < services.length; i++) {
    ...
    await qInsert(
      "services",
      ["slug", "title", "tagline", "description", "icon", "sort_order"],
...
async function seedClients() {
  ...
  for (let i = 0; i < clients.length; i++) {
    ...
    await qInsert(
      "clients",
      ["slug", "name", "kind", "logo_url", "featured", "sort_order"],
...
async function seedTestimonials() {
  ...
  const sysRows = await q<{ slug: string; id: string }>(
    "SELECT slug, id FROM `systems`",
  );
  const sysIds = sysRows.reduce<Record<string, string>>((acc, r) => {
    acc[r.slug] = r.id;
    return acc;
  }, {});
...
async function seedStatistics() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    await qInsert(
      "statistics",
      ["key", "label", "value", "suffix", "scope", "sort_order"],
...
async function seedTeam() {
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    await qInsert(
      "team_members",
      ["slug", "name", "role", "bio", "specialties", "socials", "sort_order"],
      [
        m.slug,
        m.name,
        m.role,
